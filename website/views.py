...
from django.views.generic import TemplateView, ListView, CreateView, View, DeleteView, DetailView,  UpdateView
...
class HomeView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            context['my_questions'] = Question.objects.filter(user=self.request.user)[:5]
            context['other_questions'] = Question.objects.filter(~Q(user=self.request.user))
...
class CreateQuestionView(RedirectIfNotLoggedInMixin, View):
    ...
    def post(self, *args, **kwargs):
        q = Question(user=self.request.user, title=self.request.POST.get('title'), 
                    description=self.request.POST.get('description'))
        q.save()
        for image in self.request.POST.get('images').split(' '):
            Image.objects.create(question=q, image=image, name=time.strftime("%d-%m-%Y--%H-%M-%S"))
...
class DetailQuestionView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if order == 'upvotes':
            ...
            answers = answers.annotate(totalvotes=Count('upvoters')+Count('downvoters')).order_by('-totalvotes')
...
class UpdateQuestionView(RedirectIfNotLoggedInMixin, UpdateView):
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        question.save()
        for image in images:
            Image.objects.create(question=question, image=image, name=time.strftime("%d-%m-%Y--%H-%M-%S"))
        return JsonResponse(None, safe=False)
...
class QuestionAnswerView(RedirectIfNotLoggedInMixin, View):
    def post(self, req, pk):
        Answer.objects.create(question=Question.objects.get(pk=pk), 
                            answer=req.POST.get('answer'),
                            answerer=req.user).save()
...
class QuestionAnswerEditView(RedirectIfNotLoggedInMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse('detail-question', args=[self.get_object().question.id, 'recent'])
...
class QuestionAnswerReplyView(RedirectIfNotLoggedInMixin, View):
    def post(self, req, qid, aid):
        reply = Reply.objects.create(answer=Answer.objects.get(pk=aid), replier=req.user, reply=req.POST.get('reply'))
        reply.save()
...
class QuestionAnswerVote(RedirectIfNotLoggedInMixin, View):
    def post(self, _, aid, voteType):
        ...
        if voteType == 'UPVOTE':
            ...
            if not answer.downvoters.filter(username=self.request.user.username).exists():
